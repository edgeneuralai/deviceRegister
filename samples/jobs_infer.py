...
import configparser
from inference_docker import InferenceDocker
...
def job_thread_fn(job_id, job_document):
    try:
        print("Publishing request to update job status on iot to IN_PROGRESS...")
        ...
        infer_proc = InferenceDocker(args.workdir, args.hardware)
        #optimize_proc.start_optimization_docker(job_document)
        if(operation=="deploy"):
            infer_proc.start_inference_docker(job_document)
        else:
            infer_proc.killjob()
        
        print("Done working on job.")

       # print("Publishing request to update job status on iot to SUCCEEDED...")
       # request = iotjobs.UpdateJobExecutionRequest(
       #     thing_name=args.thing_name,
       #     job_id=job_id,
       ...
       # publish_future.add_done_callback(on_publish_update_job_execution)
        ...
        #print("Publishing custom request to update job status to SUCCEEDED...")
        #custom_request = iotjobs.UpdateJobExecutionRequest(
        #       thing_name=args.thing_name,
        #       job_id=job_id,
        #       status=job_id+":"+iotjobs.JobStatus.SUCCEEDED)
                
        #publish_custom_future = jobs_client.publish_to_custom_topic(custom_request, mqtt.QoS.AT_LEAST_ONCE)
...
#def on_calling_custom_topic():
#    print("Calling custom topic")
...
if __name__ == '__main__':
    ...
    args.endpoint = configur.get("SETTINGS","IOT_ENDPOINT")
    ...
    optmize_proc = InferenceDocker(args.workdir, args.hardware)
    ...
    try:
        ...
        changed_subscription_request = iotjobs.NextJobExecutionChangedSubscriptionRequest(thing_name=args.thing_name)
        ...
        #custom_topic = '$aws/rules/rule_deploy_new/status'
        #print("Subscribing to topic '{}'...".format(custom_topic))
        #subscribe_future, packet_id = mqtt_connection.subscribe(
        #    topic=custom_topic,
        #    qos=mqtt.QoS.AT_LEAST_ONCE,
        #    callback=on_calling_custom_topic)
